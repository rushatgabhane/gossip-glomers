...
Message::Message(const std::string &sender, const std::string &recipent, const std::string &type,
                 const boost::json::object &body, const boost::optional<int> &msgId,
...
void Node::handleMessage(const Message &request) {
    ...
    if (auto handlersIter = _messageHandlers.find(msgType);
        ...
        std::async(std::launch::async, [&]() { handlersIter->second->handle(request); });
    } else {
...
void Node::_init(const Message &request) {
    ...
    Message response{sender, recipient, msgType, responseBody, msgId, msgInReplyTo};
    response.send();
}
